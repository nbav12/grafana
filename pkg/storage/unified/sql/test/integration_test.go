...
func newServer(t *testing.T, cfg *setting.Cfg, isHA bool) (sql.Backend, resource.ResourceServer) {
	t.Helper()
	if cfg == nil {
		cfg = setting.NewCfg()
	}
...
func TestIntegrationBackendHappyPath(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		backend, server := newServer(t, nil, tc.isHA)
...
func TestIntegrationBackendWatchWriteEventsFromLastest(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			backend, _ := newServer(t, nil, tc.isHA)
